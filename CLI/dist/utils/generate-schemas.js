...
export default async function generateSchemas(options) {
    ...
    const schemas = JSON.parse(await fs.readFile(path.resolve(process.cwd(), 'schemas.json'), 'utf-8'));
    ...
    schemas.forEach(async (schema) => {
        ...
        try {
            await fs.writeFile(path.resolve(schemaPath, `${schemaName}.js`), generateSchema(schema));
        }
        catch (e) {
...
async function generateMain(schemaPath, schemas) {
    try {
        await fs.writeFile(path.resolve(schemaPath, 'index.ts'), schemasMain(schemas));
    }
    catch (e) {
        console.log(`${chalk.red.bold('Error:')} ${e.message}`);
    }
